...
2. **Пересоберите веб-приложение с правильным API URL (если еще не собрано):**
   ```bash
   cd web
   # Создайте .env файл для Vite (если его нет)
   ...
   **Важно:** Убедитесь, что `web/dist` содержит собранные файлы перед запуском docker-compose.
...
2. Если были изменения во frontend, пересоберите веб-приложение:
   ```bash
   cd web
